...
def remind_changes(from_user, user_id, coeff):
    ...
    for v in sorted(list(from_user[user_id]['interval']), key=lambda tup: (coeff[tup[1]], tup[2])):
        keyboard.add(types.InlineKeyboardButton(f'{v[0]} - {v[1]} {v[2]}', callback_data=f'remind,{v[3]}'))
        remind_buttons.add(v[0])
    for i, v in enumerate(sorted(remind_buttons)):
        keyboard.add(types.InlineKeyboardButton(f'Добавить напоминание к "{v}"', callback_data=f'goal,{i}'))
